...
task run_souporcell {
    ...
    command {
        ...
        souporcell_call_args = ['souporcell_pipeline.py', '-i', '~{input_bam}', '-b', '~{input_rna}' , '-f', 'genome_ref/fasta/genome.fa', '-t', '~{num_cpu}', '-o', 'result', '-k', '~{num_clusters}','--ignore','True','--min_alt','~{min_alt}','--min_ref','~{min_ref}']
    ...
    output {
        ...
        File clusters_tmp = "result/clusters_tmp.tsv"
